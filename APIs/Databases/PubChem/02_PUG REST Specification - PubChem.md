...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/<input specification>/<operation 

specification>/[<output specification>][?<operation_options>] 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 2/31

<domain> = substance | compound | assay | gene | protein | pathway | 

taxonomy | cell | <other inputs> 

compound domain <namespace> = cid | name | smiles | inchi | sdf | inchikey 

| formula | <structure search> | <xref> | <mass> | listkey | <fast search> 
...
fastsubstructure | fastsuperstructure } / { smiles | smarts | inchi | sdf 
...
<mass> = { molecular_weight | exact_mass | monoisotopic_mass } / { equals 

| range } / value_1 { / value 2 } 

substance domain <namespace> = sid | sourceid/<source id> | 

sourceall/<source name> | name | <xref> | listkey 

<source name> = any valid PubChem depositor name 

assay domain <namespace> = aid | listkey | type/<assay type> | 

sourceall/<source name> | target/<assay target> | activity/<activity 
...
annotations/[sourcename/<source name> | heading/<heading>] | 

classification | standardize | periodictable   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 3/31

<identifiers> = comma-separated list of positive integers (e.g. cid, sid, 

aid) or identifier strings (source, inchikey, formula); in some cases only 

a single identifier string (name, smiles, xref; inchi, sdf by POST only) 

For example, to access CID 2244 (aspirin), one would construct the first part of the URL this way: 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/2244/<operation 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceid/DTP.NCI/<oper 

ation specification>/[<output specification>] 
...
compound domain <operation specification> = record | <compound property> | 

synonyms | sids | cids | aids | assaysummary | classification | <xrefs> | 

description | conformers 

<compound property> = property / [comma-separated list of property tags] 

substance domain <operation specification> = record | synonyms | sids | 

cids | aids | assaysummary | classification | <xrefs> | description 

<xrefs> = xrefs / [comma-separated list of xrefs tags] 

assay domain <operation specification> = record | concise | aids | sids | 

cids | description | targets/<target type> | <doseresponse> | summary | 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 4/31

<doseresponse> = doseresponse/sid 

gene domain <operation specification> = summary | aids | concise | pwaccs 

protein domain <operation specification> = summary | aids | concise | 

pwaccs 

pathway domain <operation specification> = summary | cids | geneids | 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/2244/property/Molec 

ularFormula,InChIKey/[<output specification>] 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/2244/property/Molecular Formula/JSON 

JSONP takes an optional callback function name (which defaults to “callback” if not specified). For example:   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 5/31

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/2244/property/Molecular Formula/JSONP?callback=my_callback 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 6/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/property/MolecularForm ula,MolecularWeight/CSV 
...
If the operation was successful, the HTTP status code will be 200 (OK). If the server encounters an error, it will return an HTTP status code that gives some indication of what went wrong; possibly along with, depending on the output format (such as in a 

tag in XML), some additional more human-readable detail message(s). The codes in the 400-range are errors on the client side, and those in the 500 range indicate a problem on the server side; the codes currently in use are: 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 7/31
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 8/31

Option  Allowed Values (default in bold)  Meaning 

record_type  2d , 3d  Type of conformer for compounds 

image_size  large , small, <width>x<height> 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceid/IBM/5F1CA2B314D3 5F28C7F94168627B29E3/ASNT 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceid/DTP.NCI/747285/SDF 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceid/DTP.NCI/747285/PN G
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 9/31
...
Connectivity SMILES (Simplified Molecular Input Line Entry System) string. Contains the connectivity layer only, and does NOT include stereochemistry or isotopes. 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 10/31
...
FeatureCount3D  Total number of 3D features (the sum of FeatureAcceptorCount3D, FeatureDonorCount3D,   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 11/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/1,2,3,4,5/property/Molec ularFormula,MolecularWeight,InChIKey/CSV 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 12/31
...
aids_type  all , active, inactive 

Type of AIDs to return, given SIDs or CIDs 

sids_type  all , active, inactive, doseresponse 

Type of SIDs to return, given AIDs 

sids_type  all,  standardized , component 
...
cids_type  all , active, inactive 

Type of CIDs to return, given AIDs 

cids_type  all,  standardized , component 

Type of CIDs to return, given SIDs 

cids_type 

original , parent, component, preferred, same_stereo, same_isotopes, same_connectivity, same_tautomer, same_parent, same_parent_stereo, same_parent_isotopes, same_parent_connectivity, same_parent_tautomer 
...
sourcename  (any substance source) 

For SIDs by name only, restrict to source       

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 13/31 Option Allowed Values (default in bold) Meaning
> hold_type live_only, live_and_on_hold , on_hold_only
> For SIDs or AIDs by sourceall, whether to include on-hold
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/listkey/xxxxxx/sids/XML 

(where ‘xxxxxx’ is the listkey from the above URL) 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceall/R%26D%20Chemical s/sids/XML?list_return=listkey 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 14/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/xref/PatentID/EP0711162A1/si ds/XML 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 15/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/1000,1001/assaysummar y/CSV 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 16/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/gene/geneid/1956,13649/summary/JSO N (by GeneID) 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/gene/genesymbol/EGFR/summary/XML 

(by genesymbol, case-insensitive and default to human) 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/gene/genesymbol/EGFR/10090/summar y/JSON  (mouse with NCBI TaxonomyID 9606) 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 17/31
...
Returns a summary of taxonomy: TaxonomyID, ScientificName, CommonName, Rank, RankedLineage, and a list of Synonyms. Valid output formats are XML, JSON(P), and   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 18/31

ASNT/B. 

Examples: 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/taxonomy/taxid/9606,10090,10116/sum mary/JSON 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/cell/cellacc/CVCL_0030,CVCL_0045/sum mary/JSON  (by  Cellosaurus  cell line accession) 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 19/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sid/1,2,3,135653256/dates/X ML?dates_type=modification,deposition,hold 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 20/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sourceall/ChEBI/xrefs/Registry ID/JSON 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 21/31

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsubstructure/inchi/ 

cids/XML (where the POST body contains “inchi=InChI=1S/C9H6N4/c1-2-10-3-6-

7(1)11-4-8-9(6)13-5-12-8/h1-5H,(H,12,13)”) 
...
MatchTautomers  boolean 

allow match to tautomers of the given structure (no longer supported) 
...
This is a special type of compound namespace input that retrieves CIDs by 2D similarity search. It requires a CID, or a SMILES, InChI, or SDF string in the URL path or   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 22/31

POST body (InChI and SDF by POST only). Valid output formats are XML, JSON(P), and ASNT/B. 

Example: 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsimilarity_2d/cid/2244/cid s/XML 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastidentity/smiles/CCCCC/ci ds/XML 
...
same_connectivity, same_tautomer, same_stereo, same_isotope, same_stereo_isotope, nonconflicting_stereo, same_isotope_nonconflicting_stereo 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 23/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastformula/C10H21N/cids/X ML 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastformula/C10H21N/cids/J SON?AllowOtherElements=true&MaxRecords=10 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 24/31

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastformula/C6H12O/cids/JS ON?list_return=cachekey 

This will produce something like: 

{"IdentifierList": { "Size": 692, "CacheKey": "is0tv08FKrkdkyiKqvJhpCDDO6PtouNSmXf4HoJm6h-Cf9Y" }}

Use that cache key string as input to a substructure search this way: 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsubstructure/smiles/C1CC CCC1/cids/JSON?cachekey=is0tv08FKrkdkyiKqvJhpCDDO6PtouNSmXf4HoJm6h-Cf9Y 
...
(where ‘xxxxx’ is the ListKey returned in the prior search request) 

Examples: 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/substructure/cid/2244/cids/X ML?StripHydrogen=true 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/identity/cid/5793/cids/TXT? identity_type=same_connectivity 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/similarity_2d/cid/2244/cids/X ML?Threshold=99 

https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/similarity_3d/cid/2244/cids/J SON   

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 25/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/classification/hnid/<integer>/<i 

d type>/<format> 

The HNID integer can be obtained from the classification browser, and is the identifier for a specific classification node. The output identifier type is case-  

> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 26/31

insensitive and must be one of: cid, compound; sid, substance; aid, bioassay; patent; pmid, pubmedid; doi; gene, geneid; protein; taxonomy, taxonomyid; pathway, pathwayid; disease, diseaseid; or cell, cellid. Note that the plural form is also accepted, e.g. "cid" or "cids". The list can also be retrieved as a cache key (but, note, not as a list key). Valid formats are TXT, XML, JSON(P), and ASNT/B. 
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/classification/hnid/4501233/patents/JSO N
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 27/31
...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/listkey/xxxxxx/cids/XML 

?listkey_start=2&listkey_count=2 where ‘xxxxxx’ is the listkey returned by 

the first URL, will return a list containing (only) CIDs 3 and 4. 
...
> 2025/10/27 01:29 PUG REST - PubChem
> https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination 28/31

2025/10/27 01:29 PUG REST - PubChem 

https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination  29/31 2025/10/27 01:29 PUG REST - PubChem 

https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination  30/31 2025/10/27 01:29 PUG REST - PubChem 

https://pubchem.ncbi.nlm.nih.gov/docs/pug-rest#section=Pagination  31/31
