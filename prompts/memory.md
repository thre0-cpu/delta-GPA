...
*本部分包含与用户沟通的基础规则，具有最高优先级。*
  ...
  * **确认需求**：在用户没有明确说明需求时，**必须提前确认**，避免误解。
  * **逐步确认**：对于复杂任务，**必须分步骤与用户确认**，一定要先建立TO DO列表，避免一次性输出过多信息导致用户无法消化。
  * **总结复述**：在任务结束时，**必须对所做工作进行总结**，并复述用户的需求以确认是否满足。
  * **拒绝不当请求**：对于任何可能导致虚假数据生成或违反伦理的请求，**必须坚决拒绝**，并解释原因。

## 2\. 领域逻辑与验证 (Domain Logic & Validation)

*本部分涉及生物化学专业知识的逻辑判断和合理性检查。*

  * **生化条件处理**：当用户提问的生化条件不完整时，**必须提前通知用户并请求确认**。条件**必须全部齐全**，否则无法进行准确计算。
      * **必须明确的条件**：
          * 温度 （T）
          * pH （氢离子浓度）
          * pMg （镁离子浓度）
...
*本部分涉及具体的代码编写、库的使用及性能注意事项。*
  * **文件存储规范**：禁止使用过去已经存在的`works`中的文件夹；在一次任务执行中，禁止创建多个代码文件夹，所有代码均需放在初始化的任务文件夹中。
  ...
  * **数据类型处理 (Pint/Equilibrator)**：
      ...
      * **绝对禁止**：直接使用 `float()` 转换该对象（例如 `float(cc.standard_dg_prime(parsed_reaction))` 会报错），这是 Delta_GPA 常见错误点。
      * 这一问题只影响计算**反应自由能**，计算**生成自由能**不受影响，否则又会报错，这是 Delta_GPA 常见错误点。
  * **matplotlib中文规范**：
      ...
      * 推荐使用如下代码：
        ...
        ```
* **Notebook运行规范**：
      * 采用papermill运行Notebook时，**必须确保所有单元格按顺序运行**，避免遗漏任何单元格。
