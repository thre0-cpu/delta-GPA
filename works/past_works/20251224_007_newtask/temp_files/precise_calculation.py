...
# 由于平衡常数相对较小，我们可以用一种不同的方法
# 假设MoS4_2浓度几乎不变，近似等于初始浓度
# 我们先尝试一个更简单的近似方法

# 设 [H2S]aq = x，这是一个待求解的值
# 从平衡关系可得（假设[MoS4_2] ≈ Mo_total）：
...
# 由气液平衡：x * V_aq + (x * H_Henry_L) * V_g = [MoOS3_2] + [MoO2S2_2] + [MoO3S_2] + [MoO4_2]
# 即：x * (V_aq + H_Henry_L * V_g) = K1*Mo_total/x + K1*K2*Mo_total/x^2 + K1*K2*K3*Mo_total/x^3 + K1*K2*K3*K4*Mo_total/x^4

# 令 A = V_aq + H_Henry_L * V_g
# 方程变为：A*x = Mo_total * (K1/x + K1*K2/x^2 + K1*K2*K3/x^3 + K1*K2*K3*K4/x^4)
# A*x^5 = Mo_total * (K1*x^4 + K1*K2*x^3 + K1*K2*K3*x^2 + K1*K2*K3*K4*x)
# A*x^5 = K1*Mo_total * x * (x^3 + K2*x^2 + K2*K3*x + K2*K3*K4)
# A*x^4 = K1*Mo_total * (x^3 + K2*x^2 + K2*K3*x + K2*K3*K4)
...
print(f"多项式: {A}*x^4 - {K1*Mo_total}*x^3 - {K1*K2*Mo_total}*x^2 - {K1*K2*K3*Mo_total}*x - {K1*K2*K3*K4*Mo_total} = 0")
...
if real_roots:
    ...
    def equation(x):
        A = V_aq + H_Henry_L * V_g
        left = A * x**4
        right = K1*Mo_total*x**3 + K1*K2*Mo_total*x**2 + K1*K2*K3*Mo_total*x + K1*K2*K3*K4*Mo_total
        return left - right
    
    # 尝试在不同区间寻找根
    try:
        x_solution = opt.brentq(equation, 1e-15, 1e-3)
        print(f"使用数值方法找到解: [H2S]aq = {x_solution:.3e}")
    except:
        print("数值方法也未能找到合适的解")
        # 使用最简单的近似方法
        print("使用最简单的近似方法：")
        # 假设只有第一步反应发生，且达到平衡
        x_simple = (K1 * Mo_total) ** 0.5  # 从 K1 = [MoOS3][H2S]/[MoS4] ≈ [H2S]^2/[MoS4] 得到
        print(f"简单近似: [H2S]aq ≈ {x_simple:.3e}")
        
        # 计算各钼物种浓度
        MoS4_2 = Mo_total
        MoOS3_2 = K1 * MoS4_2 / x_simple
        MoO2S2_2 = K2 * MoOS3_2 / x_simple
        MoO3S_2 = K3 * MoO2S2_2 / x_simple
        MoO4_2 = K4 * MoO3S_2 / x_simple
        ...
        # 计算H2S的其他存在形式
        HS_neg = x_simple * Ka1 / H_plus
        S2_neg = HS_neg * Ka2 / H_plus
        
        # 计算气相中H2S的分压
        H2S_gas_conc = H_Henry_L * x_simple
        H2S_gas_pressure = H2S_gas_conc * R * T
        
        print(f"\n近似含硫物种浓度/气压 (mol/L 或 atm):")
        print(f'[H2S(aq)] = {x_simple:.3e}')
        print(f'[HS^-] = {HS_neg:.3e}')
        print(f'[S^2-] = {S2_neg:.3e}')
        print(f'H2S(g)分压 = {H2S_gas_pressure:.3e}')
        ...
        # 含硫不含钼物种（浓度或气压）
        S_species = [x_simple, HS_neg, S2_neg, H2S_gas_pressure]
        S_species_names = ['H2S(aq)', 'HS^-(aq)', 'S^2-(aq)', 'H2S(g)']
        ...
        print(f'\n近似含钼物种浓度乘积: {Mo_product:.3e}')
        print(f'近似含硫不含钼物种浓度/气压乘积: {S_product:.3e}')
        print(f'近似浓度/气压乘积比值: {ratio:.3e}')