...
xylene_identifiers = [
    ...
    "p-xylene", 
    "dimethylbenzene",
    "C01687",  # KEGG ID for xylene
    "1330-20-7",  # CAS number for xylene
    "CC1=CC=C(C=C1)C",  # SMILES for p-xylene
    "CC1=CC(=CC=C1)C",  # SMILES for o-xylene
    "C1(=CC=CC=C1)C(C)C"  # SMILES for m-xylene
]

print("尝试计算二甲苯在298K，pH=10，pMg=6条件下的生成自由能:")
print("条件: T=298K, pH=10, pMg=6, I=0.25M")

found = False
for identifier in xylene_identifiers:
    print(f"\n尝试标识符: {identifier}")
    try:
        dGf, std = standard_dgf_prime_CC(identifier, cc, p_h=10, p_mg=6, T=298)
        if not (np.isnan(dGf) or np.isnan(std)):
            print(f"成功找到 {identifier}: Δ_fG'° = {dGf:.2f} ± {std:.2f} kJ/mol")
            found = True
        else:
            print(f"无法计算 {identifier} 的生成自由能")
    except Exception as e:
        print(f"处理 {identifier} 时发生错误: {str(e)}")

if not found:
    print("\n未能找到二甲苯的任何有效标识符，无法计算生成自由能。")
    print("这可能是因为二甲苯不是生物体系中的常见化合物，不在eQuilibrator数据库中。")