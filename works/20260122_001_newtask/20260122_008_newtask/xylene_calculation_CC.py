...
xylene_identifiers = [
    ...
    "m-xylene",         # 间二甲苯
    "p-xylene",         # 对二甲苯
    "dimethylbenzene",  # 二甲苯的另一个名称
    "C6H4(CH3)2",       # 化学式
    "1330-20-7",        # CAS号
    "Cc1ccccc1C",       # o-xylene SMILES
    "Cc1cccc(C)c1",     # m-xylene SMILES
    "Cc1ccc(C)cc1",     # p-xylene SMILES
    "KEGG:C01687"       # KEGG ID
]
...
for identifier in xylene_identifiers:
    ...
    try:
        ...
        if np.isnan(dGf):
            ...
            print(f"  {identifier} Δ_fG'°: {dGf:.2f} ± {std:.2f} kJ/mol")
            results[identifier] = (dGf, std)
    except Exception as e:
        print(f"  计算 {identifier} 时出错: {str(e)}")

print("\n" + "="*50)
print("计算完成！")
print("汇总结果:")
for identifier, (dGf, std) in results.items():
    print(f"  {identifier}: {dGf:.2f} ± {std:.2f} kJ/mol")